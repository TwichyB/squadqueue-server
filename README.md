...
โปรเจกต์นี้ส่งอีเมลผ่าน [Resend](https://resend.com) (HTTP API) แทนการต่อ SMTP ตรงๆ แบบ Gmail เพราะแพลนฟรีของ Render (และผู้ให้บริการฟรีอีกหลายเจ้า) บล็อกการเชื่อมต่อออกไปที่พอร์ต SMTP (25/465/587) ทั้งหมด ต่อให้ตั้งค่า Gmail ถูกทุกอย่างก็จะเจอ `ETIMEDOUT` เพราะเชื่อมต่อไม่ได้ตั้งแต่ต้น — Resend ส่งผ่าน HTTPS ธรรมดาเลยไม่ติดปัญหานี้

1. สมัครบัญชีฟรีที่ https://resend.com
2. สร้าง API key ที่ https://resend.com/api-keys
3. นำค่าที่ได้ไปใส่ใน `.env` (หรือ Environment ของ Render):
   ```
   RESEND_API_KEY=re_xxxxxxxxxxxx
   RESEND_FROM=SquadQueue <onboarding@resend.dev>
   ```
...
5. ถ้าไม่ตั้งค่า `RESEND_API_KEY` ไว้ (ปล่อยว่าง) เซิร์ฟเวอร์จะไม่ส่งอีเมลจริง แต่จะพิมพ์ลิงก์ยืนยันออกทาง console log แทน — สะดวกสำหรับทดสอบในเครื่อง แต่ผู้ใช้จริงจะเข้าระบบไม่ได้ถ้าไม่มีใครส่งลิงก์ให้

**ข้อจำกัดสำคัญ**: ก่อนยืนยันโดเมนของตัวเองกับ Resend (ที่ https://resend.com/domains) บัญชี Resend จะส่งอีเมลได้แค่ไปหาอีเมลที่ใช้สมัคร Resend เองเท่านั้น — ส่งหาผู้ใช้จริงคนอื่นจะโดน Resend ตอบ 403 กลับมา (เซิร์ฟเวอร์จะจับ error นี้ไว้ไม่ให้แอปพัง แต่อีเมลจะไม่ถึงผู้ใช้คนนั้นจริงๆ) เหมาะสำหรับทดสอบระบบด้วยบัญชีตัวเองไปก่อน ถ้าจะเปิดให้คนอื่นสมัครใช้งานจริงต้องมีโดเมนของตัวเองมายืนยันกับ Resend ก่อน
