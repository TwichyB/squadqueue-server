...
## ตั้งค่ายืนยันอีเมล (SMTP2GO API)
...
โปรเจกต์นี้ส่งอีเมลผ่าน [SMTP2GO](https://www.smtp2go.com) (HTTP API) แทนการต่อ SMTP ตรงๆ แบบ Gmail เพราะแพลนฟรีของ Render (และผู้ให้บริการฟรีอีกหลายเจ้า) บล็อกการเชื่อมต่อออกไปที่พอร์ต SMTP (25/465/587) ทั้งหมด ต่อให้ตั้งค่า Gmail ถูกทุกอย่างก็จะเจอ `ETIMEDOUT` เพราะเชื่อมต่อไม่ได้ตั้งแต่ต้น — SMTP2GO ส่งผ่าน HTTPS ธรรมดาเลยไม่ติดปัญหานี้

1. สมัครบัญชีฟรีที่ https://www.smtp2go.com
2. ยืนยันอีเมลผู้ส่งที่ https://app.smtp2go.com/senders/ — ใช้อีเมลของตัวเองได้เลย **ไม่ต้องมีโดเมน** (ต่างจาก Resend) กด "Add Single Email" ใส่อีเมล แล้วไปกดลิงก์ยืนยันที่ส่งเข้ากล่องจดหมาย
3. สร้าง API key ที่ https://app.smtp2go.com/apikeys/
4. นำค่าที่ได้ไปใส่ใน `.env` (หรือ Environment ของ Render):
   ```
   SMTP2GO_API_KEY=api-xxxxxxxxxxxxxxxxxx
   SMTP2GO_SENDER=SquadQueue <อีเมลที่ยืนยันไว้ในข้อ 2>
   ```
5. ตั้งค่า `APP_BASE_URL` ให้ตรงกับโดเมนจริงที่ผู้ใช้เข้าเว็บ (เช่น `https://squadqueue-server.onrender.com`) — ค่านี้สำคัญมาก เพราะถูกใช้สร้างลิงก์ยืนยันในอีเมล ถ้าตั้งผิดหรือไม่ตั้งเลย ลิงก์อาจชี้ไปผิดโดเมน (โดยเฉพาะเมื่อรันหลัง proxy อย่าง Render)
6. ถ้าไม่ตั้งค่า `SMTP2GO_API_KEY`/`SMTP2GO_SENDER` ไว้ (ปล่อยว่าง) เซิร์ฟเวอร์จะไม่ส่งอีเมลจริง แต่จะพิมพ์ลิงก์ยืนยันออกทาง console log แทน — สะดวกสำหรับทดสอบในเครื่อง แต่ผู้ใช้จริงจะเข้าระบบไม่ได้ถ้าไม่มีใครส่งลิงก์ให้

**ข้อดีเทียบกับ Resend**: พอยืนยันอีเมลผู้ส่งแล้ว (ข้อ 2) จะส่งหาผู้ใช้จริงคนไหนก็ได้ทันที ไม่ถูกจำกัดให้ส่งได้แค่หาอีเมลตัวเองเหมือน Resend แพลนฟรี (ที่ต้องมีโดเมนของตัวเองมายืนยันก่อนถึงจะส่งหาคนอื่นได้) แลกมาด้วยโอกาสตกสแปมสูงกว่าเล็กน้อยเพราะไม่มีการยืนยันระดับโดเมน (SPF/DKIM) เต็มรูปแบบ ถ้าเจอปัญหาอีเมลเข้าสแปมบ่อยในอนาคต ค่อยพิจารณายืนยันโดเมนของตัวเองกับ SMTP2GO เพิ่มได้ทีหลัง
